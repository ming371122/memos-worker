...
async function handleApiRequest(request, env) {
	...
	// 匹配获取所有已分享文章的公开 API /api/public/shared-notes
	if (pathname === '/api/public/shared-notes' && request.method === 'GET') {
		return handlePublicSharedNotesRequest(env);
	}
	// --- Memos 分享公开路由 ---
...
async function handleSearchRequest(request, env) {
	...
	try {
		...
		// 为每个笔记添加分享信息
			const notesWithShareInfo = await Promise.all(notes.map(async (note) => {
			...
			return jsonResponse({ notes: notesWithShareInfo, hasMore });
	} catch (e) {
...
/**
 * 处理获取所有已分享文章列表的请求
 * GET /api/public/shared-notes
 */
async function handlePublicSharedNotesRequest(env) {
	try {
		const db = env.DB;
